...
--><pattern xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ahf="http://www.antennahouse.com/names/XSLT/Functions/Document" id="fo-fo">
  ...
  <rule context="fo:change-bar-begin">
    ...
    <report test="exists(@change-bar-class) and not(@change-bar-class = following::fo:change-bar-start/@change-bar-class)" role="Warning">fo:change-bar-end と一致する対を成らない <value-of select="name()"/> は、文書の最後に一致する change-bar-end と想定します。</report>
  </rule>

  <rule context="fo:change-bar-end">
    <!-- https://www.w3.org/TR/xsl/#fo_change-bar-end -->
    <report test="exists(@change-bar-class) and not(@change-bar-class = preceding::fo:change-bar-start/@change-bar-class)" role="Warning">fo:change-bar-begin と一致する対を成らない <value-of select="name()"/> は、無効にします。</report>
  </rule>

...
--><pattern xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ahf="http://www.antennahouse.com/names/XSLT/Functions/Document" id="fo-fo">
  ...
  <rule context="fo:root">
    ...
    <assert id="fo_root-001" test="exists(descendant::fo:page-sequence)"> fo:rootの少なくとも一つのfo:page-sequenceの子孫でなくてはいけません。</assert>
  </rule>
  ...
  <rule context="fo:*/@column-count | fo:*/@number-columns-spanned">
    ...
    <report test="local-name($expression) = 'Number' and                   (exists($expression/@is-positive) and $expression/@is-positive eq 'no' or                    $expression/@is-zero = 'yes' or                    exists($expression/@value) and not($expression/@value castable as xs:integer))" id="column-count" role="Warning"><value-of select="local-name()"/>=&quot;<value-of select="."/>&quot; は正の整数でなければなりません。非正又は非整数値は一番近い整数値に丸められます。</report>
  </rule>
  ...
  <rule context="fo:*/@flow-name">
    ...
    <assert test="count(../../*/@flow-name[. eq current()]) = 1">flow-name=&quot;<value-of select="."/>&quot; は fo:page-sequence内で特殊でなくてはいけません。</assert>
  ...
  <rule context="fo:*/@flow-name-reference">
    ...
    <assert test="count(ancestor::fo:flow-map//fo:flow-name-specifier/@flow-name-reference[. eq current()]) = 1">flow-name-reference=&quot;<value-of select="., ancestor::fo-flow-map//fo:flow-name-specifier/@flow-name-reference[. eq current()]"/>&quot; は fo:flow-map内で特殊でなくてはいけません。</assert>
  ...
  <rule context="fo:*/@overflow">
    ...
    <report test=". eq 'repeat' and ../@absolute-position = ('absolute', 'fixed')" role="Warning">overflow=&quot;<value-of select="."/>&quot; の絶対位置の領域は「auto」を処理します。</report>
  </rule>
